...
//        cudaStreamDestroy(stream_);
    ...
    void Inferencer::init(std::string& engine_path, Logger& logger) {
        ...
        prepare_buffers(engine_, &gpu_buffers_[0],
                        &gpu_buffers_[1], &cpu_output_buffer_);
    }
    ...
    void Inferencer::detect(Mat &frame) {
        ...
        cudaProprecess_.cuda_batch_preprocess(frame, gpu_buffers_[0], kInputW,
                              kInputH, stream);
    ...
    void Inferencer::infer(IExecutionContext& context, cudaStream_t& stream, void **gpu_buffers, float *output, int batchsize) {
        context.enqueue(batchsize, gpu_buffers, stream, nullptr);
        CUDA_CHECK(cudaMemcpyAsync(output, gpu_buffers[1], batchsize * K_OUTPUT_SIZE * sizeof(float), cudaMemcpyDeviceToHost, stream));
        cudaStreamSynchronize(stream);
    }

    void Inferencer::deserialize_engine(std::string &engine_name, IRuntime** runtime, ICudaEngine** engine, IExecutionContext** context) {
