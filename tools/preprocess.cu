                     ...
                     float* mean, float* std, cudaStream_t stream)
{
  ...
  int blocks = ceil(jobs / (float)threads);
  warpaffine_kernel<<<blocks, threads, 0, stream>>>(img_buffer_device, src_width * 3, src_width, src_height, dst,
                                                    dst_width, dst_height, 128, d2s, jobs, mean, std);
}
...
void cuda_batch_preprocess(std::vector<cv::Mat>& img_batch, float* dst, int dst_width, int dst_height, float* mean,
                           float* std, cudaStream_t stream)
{
  int dst_size = dst_width * dst_height * 3;
  for (size_t i = 0; i < img_batch.size(); i++)
  {
