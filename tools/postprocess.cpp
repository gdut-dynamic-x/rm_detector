...
cv::Rect get_rect(cv::Mat& img, float bbox[4])
{
  ...
  int x = round(l) < 0 ? 0 : round(l);
  int y = round(t) < 0 ? 0 : round(t);
  int w = (round(r - l) + round(l) < img.cols) ? round(r - l) : img.cols - x - 1;
  int h = (round(b - t) + round(t) < img.rows) ? round(b - t) : img.rows - y - 1;
