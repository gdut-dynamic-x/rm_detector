...
cv::Rect get_rect(cv::Mat& img, float bbox[4]) {
  ...
  cv::Rect rect = cv::Rect(round(l), round(t), round(r - l), round(b - t));
    if (rect.tl().x < 0)
        rect.x = 0;
    if (rect.tl().y < 0)
        rect.y = 0;
...
void batch_nms(std::vector<Detection>& res_batch, float *output, float conf_thresh, float nms_thresh) {
    nms(res_batch, output, conf_thresh, nms_thresh);
}
